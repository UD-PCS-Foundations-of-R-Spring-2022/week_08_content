...
### `r emo::ji("world_map")` How do I use `maps` to iterate?<br>
...
**By chunk**
````
```{r optional name, echo=T, fig.height = 8}`r ''`
code
```
````
...
> - `r num_ncc` facilities in New Castle County
> - `r num_kent` facilities in Kent County
> - `r num_sussex` facilities in Sussex County
]
...
```{r leaflet-example-out, ref.label="leaflet-example", eval=TRUE, echo=FALSE, fig.dim=c(3, 5), out.width="100%"}
```
]
---
...
```{r plotly-example-out, ref.label="plotly-example", eval=TRUE, echo=FALSE, fig.dim=c(3, 5), out.width="100%"}
```
]
...
```{r viridis-example-out, ref.label="viridis-example", eval=TRUE, echo=FALSE, fig.dim=c(3, 5), out.width="100%"}
```
]
...
<br>
<br>
# `r emo::ji("clipboard")` [Survey](https://docs.google.com/forms/d/e/1FAIpQLSfl3q9fIl6A1u-znvXde7EGER_J9KE6li2gzg3_acpifq1m8Q/viewform)
<br>
<br>
<br>
